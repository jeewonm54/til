...
> 크기를 조정하고 싶으면 -> 데이터 패널에서 수익(구간 차원) 우클릭 -> 편집 -> 구간 차원 크기를 값 입력으로 전환
...
> 연결에 추출 클릭 -> 추출 옆 편집 단추에서 추출 필터 추가 -> 편집 버튼을 눌러서 들어가서 -> 추가를 눌러 밑에 주문 날짜의 년에서 해당 년만 선택하여 필터를 생성하고 -> 밑에 주소 눌러 어느 폴더에 저장할지 

데이터 원본 : 작업을 위한 데이터 중 일부만 워크 스페이스에 불러올 때 사용

컨텍스트 : 필터 중 상위 필터
- 여러가지 필터를 작동시켰을 때 컨텍스트 필터로 지정해 주게 되면 다른 필터가 컨텍스트 필터에 종속되어 작동함
- 종속 필터를 필터 선반에 두기 이전에 컨텍스트 필터를 먼저 설정하면 쿼리 속도가 향상됨
...
- 그룹을 이용해 수동으로 필드에 있는 항목들 묶을 수 있음
- 기존 데이터 원본에 없는 사용자 지정 그룹 필드를 만들 수 있음

그룹을 만드는 방법

1. 뷰에서 그룹 만들기
> 뷰에서 드래그 해서 선택 -> 마우스 우클릭 그룹 선택

2. 항목별로 묶을 필드를 선택해 만드는 방법
> "제품 이름" 필드 위에 마우스 우클릭해 "만들기"에서 그룹 클릭 -> Shift 누른 채 전부 선택해 그룹 클릭  
...
```
년(배송 날짜)를 컨텍스트 필터 설정하여 2021년, 2022년의 매출 상위 10개 도시들을 표시했다. 
```
...
```
'제품 이름' 우클릭 -> 만들기 -> 그룹 편집 -> 우측 아래 찾기 클릭 -> 시작 문자 입력 -> 각 그룹 생성  
```
...
```
1. Product Name Group을 행 선반, 주문 id를 우클릭 열 선반에 드래그하여 카운트 선택
2. Product Name Group을 Ctrl 필터로 드래그해서 기타 해제
3. 국가/지역 필드를 색상 마크로 드래그
완성 !
```
